...
const monthPattern = /(?<year>\d{4})[-/年.](?<month>\d{1,2})(?:[-/月.]|$)|(?<monthText>[A-Za-z]{3})[- ](?<yearText>\d{2,4})/;
...
export function parsePeriod(raw, frequencyHint) {
    const text = raw.trim();
    ...
    if (frequencyHint === "monthly" || monthPattern.test(text)) {
        ...
        if (match?.groups) {
            ...
            const year = rawYear.length === 2 ? (Number(rawYear) >= 70 ? 1900 + Number(rawYear) : 2000 + Number(rawYear)) : Number(rawYear);
