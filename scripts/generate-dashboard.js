...
async function main() {
    ...
    const cards = await Promise.all(cardDefinitions.map(async (card) => {
    ...
    const dashboard = cards.some((card) => card.metrics.some((metric) => metric.latest))
    ...
    const manifest = {
        ...
        history_paths: Object.fromEntries(dashboard.cards.map((card) => [card.id, `./history/${card.id}.json`]))
    };
    dashboardManifestSchema.parse(manifest);
    await writeFile(path.join(publicDataDir, "dashboard.json"), `${JSON.stringify(dashboard, null, 2)}\n`, "utf-8");
    await writeFile(path.join(publicDataDir, "manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf-8");
    await Promise.all(dashboard.cards.map((card) => writeFile(path.join(historyDir, `${card.id}.json`), `${JSON.stringify(card, null, 2)}\n`, "utf-8")));
}
async function resolveCardMetrics(cardId, previousMetrics) {
    switch (cardId) {
        case "employment":
            return Promise.all([
                resolveMetric("unemployment", previousMetrics, async () => fetchCenstatdSeries({
                    tableId: "210-06101",
                    filters: { SEX: "", freq: "M3M", sv: "UR" },
                    frequency: "monthly",
                    unit: "%"
                })),
                resolveMetric("underemployment", previousMetrics, async () => fetchCenstatdSeries({
                    tableId: "210-06101",
                    filters: { SEX: "", freq: "M3M", sv: "UDR" },
                    frequency: "monthly",
                    unit: "%"
                })),
                resolveMetric("labour_force", previousMetrics, async () => fetchCenstatdSeries({
                    tableId: "210-06101",
        ...
        case "consumption-travel":
            return Promise.all([
                resolveMetric("retail_sales", previousMetrics, async () => fetchCenstatdSeries({
                    tableId: "620-67001",
                    filters: { freq: "M", sv: "VAL_RS" },
                    frequency: "monthly",
                    unit: "百萬港元"
                })),
                resolveMetric("visitor_arrivals", previousMetrics, async () => fetchCenstatdSeries({
                    tableId: "650-80001",
        ...
        case "population":
            return Promise.all([
                resolveMetric("population_total", previousMetrics, async () => fetchCenstatdSeries({
                    tableId: "110-01002",
                ...
                resolveMetric("live_births", previousMetrics, async () => fetchCsvSeries({
                    url: URLs.birthsCsv,
                    frequency: "annual",
                    unit: "人",
                    dateColumns: ["年份", "year"],
                ...
                resolveMetric("median_age", previousMetrics, async () => fetchCenstatdSeries({
                    tableId: "110-01004",
        ...
        case "housing":
            return Promise.all([
                resolveMetric("private_price_index", previousMetrics, async () => fetchCsvSeries({
                ...
                resolveMetric("private_rent_index", previousMetrics, async () => fetchCsvSeries({
                    url: URLs.rvdRentCsv,
                    frequency: "monthly",
                    unit: "指數",
                    dateColumns: ["date", "month", "月份"],
                ...
                resolveMetric("private_completions", previousMetrics, async () => fetchCsvSeries({
                    url: URLs.rvdCompletionsCsv,
                    frequency: "annual",
                    unit: "伙",
                    dateColumns: ["year", "Year", "年份"],
                    valueSelector: (row) => {
                        const entries = Object.entries(row).filter(([header]) => !/(year|年份|date|district|備註|remarks|region|class)/i.test(header));
        ...
        case "fiscal":
            return Promise.all([
                resolveMetric("government_revenue", previousMetrics, async () => fetchCsvSeries({
                ...
                resolveMetric("government_expenditure", previousMetrics, async () => fetchCsvSeries({
                    url: URLs.fstbOverviewCsv,
                    frequency: "annual",
                    unit: "百萬港元",
                    dateColumns: ["財政年度"],
                ...
                resolveMetric("fiscal_reserves", previousMetrics, async () => fetchCsvSeries({
                    url: URLs.fstbOverviewCsv,
                    frequency: "annual",
                    unit: "百萬港元",
                    dateColumns: ["財政年度"],
...
async function resolveMetric(metricId, previousMetrics, loader) {
    ...
    try {
        ...
        return fallbackMetric(config, previousMetrics[metricId], error instanceof Error ? error.message : "未知抓取錯誤");
    }
}
async function resolveDerivedMetric(metricId, previousMetrics, loader) {
    return resolveMetric(metricId, previousMetrics, loader);
}
function deriveYearOnYear(series, unit, lookback) {
    ...
    return derived.filter((point) => point !== undefined);
}
...
function indexPreviousMetrics(payload) {
    if (!payload) {
        return {};
    }
    return Object.fromEntries(payload.cards.flatMap((card) => card.metrics.map((metric) => [metric.id, metric])));
}
async function saveRawSnapshot(metricId, payload) {
    await writeFile(path.join(artifactsDir, `${metricId}.json`), `${JSON.stringify(payload, null, 2)}\n`, "utf-8");
}
main().catch((error) => {
    console.error(error);
    process.exitCode = 1;
